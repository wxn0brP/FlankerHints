...
import { hintsContainer } from "./vars.js";
let activeHints = [];
let isActive = false;
...
export function activate(key) {
    ...
    const hints = [];
    const fkContainers = document.querySelectorAll(`[data-fk]`);
    fkContainers.forEach(container => {
        const containerKey = container.getAttribute("data-fk");
        if (containerKey !== targetKey)
            return;
        const children = Array.from(container.querySelectorAll("a[href], button, input, select, textarea, [tabindex], [role]")).filter(el => isVisible(el));
        children.forEach((child, index) => {
            if (index < alphabet.length) {
                const hint = createHint(child, alphabet[index], getDefaultAction(child));
                hints.push(hint);
            }
        });
    });
    const fkElements = document.querySelectorAll(`[data-fk]`);
    ...
    if (config.autoGenerate && targetKey === config.keys.links) {
        const links = Array.from(document.querySelectorAll("a[href]")).filter(el => isVisible(el) && !el.hasAttribute("data-fk"));
    ...
    if (config.autoGenerate && targetKey === config.keys.inputs) {
        const inputs = Array.from(document.querySelectorAll("input, select, textarea")).filter(el => isVisible(el) && !el.hasAttribute("data-fk"));
    ...
    if (config.autoGenerate && targetKey === config.keys.buttons) {
        const buttons = Array.from(document.querySelectorAll("button, [role=button]")).filter(el => isVisible(el) && !el.hasAttribute("data-fk"));
...
function createHint(element, key, action) {
